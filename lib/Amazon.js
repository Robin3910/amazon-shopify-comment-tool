...
class AmazonScraper {
    ...
    async startScraper() {
        ...
        // console.log('before spinner start');
        // if (this.cli) {
        //     spinner.start();
        // }
    ...
    async mainLoop() {
        ...
        const reqCountArr = Array.from({ length: this.asyncPage }, (_, k) => k + 1);
        console.log('loop: ', reqCountArr.length);
        for (const item of reqCountArr) {
            console.log('grab time: ', item);
            const body = await this.buildRequest(this.bulk ? item : this.searchPage);
            console.log('grab body finish: ', item);
            if (this.scrapeType === 'asin') {
    ...
    async buildRequest(page) {
        ...
        try {
            console.log('send grab request');
            const response = await this.httpRequest(options);
            console.log('get grab response');
     ...
     */
    grabReviews(body) {
        console.log('start collect reviews');
        console.log(this.product_handle);
        ...
        for (let key in scrapingResult) {
            scrapingResult[key].picture_urls = '';
            // const search = $(`#${key} .review-image-tile`);
            // try {
            //     let picUrls = '';
            //     let index = 0;
            //     while(search[index]) {
            //         picUrls += `${search[index].attribs.src},`;
            //         index ++;
            //     }
            //     scrapingResult[key].picture_urls = picUrls.slice(0,picUrls.length - 1);
            // } catch (error) {
            //     continue;
            // }
        }
        ...
        for (let key in scrapingResult) {
            ...
            try {
                ...
                if (scrapingResult[key].review_data) {
                    ...
                    const date = new Date(scrapingResult[key].date.date);
                    scrapingResult[key].review_date = `${date.toJSON().substr(0, 19).replace('T', ' ')} UTC`;
                }
            } catch (error) {
                continue;
            }
        ...
        for (let key in scrapingResult) {
            ...
            try {
                ...
                scrapingResult[key].reviewer_name = search[0].children[0].data;
            } catch (error) {
                continue;
            }
        }
        ...
        for (let key in scrapingResult) {
            ...
            try {
                ...
                scrapingResult[key].body = $(search[0]).text();
            } catch (error) {
                continue;
            }
        }
        ...
        for (let key in scrapingResult) {
            scrapingResult[key].product_handle = this.product_handle;
            // scrapingResult[key].reviewer_email = '';
            scrapingResult[key].product_id = '';
            scrapingResult[key].reply = '';
